...
class TestCompareMetrics:
    ...
    def test_candidate_better(self):
        ...
        active = {"num_interactions": 100, "average_feedback_score": 0.80}
        candidate = {"num_interactions": 50, "average_feedback_score": 0.85}
    ...
    def test_candidate_worse(self):
        ...
        active = {"num_interactions": 100, "average_feedback_score": 0.85}
        candidate = {"num_interactions": 50, "average_feedback_score": 0.80}
    ...
    def test_missing_feedback_scores(self):
        ...
        active = {"num_interactions": 100, "average_feedback_score": None}
        candidate = {"num_interactions": 50, "average_feedback_score": 0.85}
    ...
    def test_candidate_no_feedback(self):
        ...
        active = {"num_interactions": 100, "average_feedback_score": 0.85}
        candidate = {"num_interactions": 50, "average_feedback_score": None}
    ...
    async def test_promote_step_increases_candidate_traffic(self):
        ...
        with patch("bindu.dspy.canary.controller.update_prompt_traffic") as mock_update:
            with patch("bindu.dspy.canary.controller.app_settings") as mock_settings:
                ...
                await promote_step(
                    active, candidate, storage=mock_storage, did="agent-1"
                )

                # Check candidate gets increased traffic (0.4 + 0.1 = 0.5)
                calls = mock_update.call_args_list
                assert any(
                    call[0][0] == 2 and abs(call[0][1] - 0.5) < 0.001 for call in calls
                )
    ...
    async def test_rollback_step_decreases_candidate_traffic(self):
        ...
        with patch("bindu.dspy.canary.controller.update_prompt_traffic") as mock_update:
            with patch("bindu.dspy.canary.controller.app_settings") as mock_settings:
                ...
                assert (
                    abs(calls[0][0][1] - 0.3) < 0.001
                )  # 0.4 - 0.1 (with floating point tolerance)
...
class TestRunCanaryController:
    ...
    async def test_run_canary_controller_no_candidate(self):
        ...
        with patch("bindu.dspy.canary.controller.PostgresStorage") as MockStorage:
            ...
            with patch(
                "bindu.dspy.canary.controller.get_candidate_prompt", return_value=None
            ):
    ...
    async def test_run_canary_controller_candidate_wins(self):
        ...
        with patch("bindu.dspy.canary.controller.PostgresStorage") as MockStorage:
            ...
            with patch(
                "bindu.dspy.canary.controller.get_candidate_prompt",
                return_value=candidate,
            ):
                with patch(
                    "bindu.dspy.canary.controller.get_active_prompt",
                    return_value=active,
                ):
                    with patch(
                        "bindu.dspy.canary.controller.promote_step"
                    ) as mock_promote:
                        result = await run_canary_controller(did="agent-1")
        ...
        with patch("bindu.dspy.canary.controller.PostgresStorage") as MockStorage:
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        "bindu.dspy.canary.controller.rollback_step"
                    ) as mock_rollback:
                        result = await run_canary_controller(did="agent-1")
    ...
    async def test_run_canary_controller_no_active(self):
        ...
        with patch("bindu.dspy.canary.controller.PostgresStorage") as MockStorage:
            ...
            with patch(
                ...
                with patch(
                    "bindu.dspy.canary.controller.get_active_prompt", return_value=None
                ):
                    result = await run_canary_controller(did="agent-1")

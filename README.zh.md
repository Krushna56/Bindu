...
## 🌐 Edge Client — 暴露您的本地 Agent

**无需部署**即可将您本地运行的 agent 暴露到互联网。
Edge Client 使用 WebSocket 连接创建到 Bindu Edge Gateway 的**临时隧道**。
...
就这么简单。
隧道会自动创建，并在终端中打印出一个**公共 URL**。您可以使用此 URL 从互联网访问您的本地 agent。

**注意：此 URL 是临时的，仅在 Edge Client 运行时有效。
当您停止或断开客户端时，隧道将被销毁，URL 将失效。
再次启动 Edge Client 将创建一个带有新公共 URL 的新隧道（旧的将不再有效）。**
...
```bash
uv run python -m bindu.edge_client [选项]
```

* `--edge-url` – Edge Gateway WebSocket URL
  *(默认：`wss://bindus.getbindu.com`)*

* `--local-port` – 本地 agent 端口
  *(默认：`3773`)*
